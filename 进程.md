...
## 孤儿与僵尸进程
- 父进程可以先于子进程结束，此时子进程成为**孤儿进程**，被init进程（PID 1）收养。
- 子进程可以先于父进程结束，此时子进程成为**僵尸进程**，直到父进程调用 `wait()` 回收。
## IO操作
### c函数调用
####  fopen
    size_t fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream);

  如果文件剩余字节数 < size * nmemb，但 >= size，会读取尽可能多的完整数据项  
  如果文件剩余字节数 < size，返回 0
...
### 系统调用
#### 输出
    fwrite(data,strlen(data),1,stdout) 输出数据
    fprintf(stdout,str);
...
pathname：文件路径  
flags：打开标志  
mode：文件权限（创建文件时使用）数字 0644 写 读 读权限  
